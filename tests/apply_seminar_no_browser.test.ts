...
describe('sync_seminars execution without Playwright Browser', () => {
  it('HTTP-only 태스크 실행 시 Chromium 브라우저 미생성 검증', async () => {
    console.log('Testing sync_seminars execution without Playwright Browser...');
    ...
    try {
      ...
      assert.strictEqual(isBrowserLaunched, false, 'Chromium browser was launched during sync_seminars!');
      assert.ok(typeof result.success === 'boolean');
      console.log('sync_seminars task result:', result);
    } finally {
      chromium.launch = originalLaunch;
      vi.restoreAllMocks();
    }

    console.log('✅ sync_seminars no-browser test passed (Chromium launch strictly verified)!');
  });
});
