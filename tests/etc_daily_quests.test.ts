...
import * as docpleDailyTaskModule from '../src/tasks/docple_daily';
...
describe('기타일일퀘스트 (etc_daily_quests) 통합 태스크 테스트', () => {
  ...
  it('인터엠디 -> 키메디 -> HMP -> 메디게이트 -> 닥플 태스크가 순차적으로 실행되고 모든 텔레그램 메시지가 전송되어야 한다', async () => {
    ...
    const docpleSpy = vi.spyOn(docpleDailyTaskModule, 'run').mockImplementation(async () => {
      executionOrder.push('docple');
      return { success: true, message: '📋 [닥플 플러스 일일 자동화 리포트]' };
    });
    ...
    // 순차 실행 순서 검증
    expect(executionOrder).toEqual(['intermd', 'keymedi', 'hmp', 'medigate', 'docple']);
    ...
    expect(docpleSpy).toHaveBeenCalledTimes(1);

    // 각 태스크의 텔레그램 메시지 발송 검증
    expect(telegramSpy).toHaveBeenCalledTimes(5);
    ...
    expect(telegramSpy).toHaveBeenNthCalledWith(5, '📋 [닥플 플러스 일일 자동화 리포트]');
    ...
    expect(result.message).toContain('닥플: ✅ 성공');
  });
  ...
  it('인터엠디 태스크에서 예외가 발생하더라도 나머지 태스크는 계속 정상 실행되어야 한다', async () => {
    ...
    const docpleSpy = vi.spyOn(docpleDailyTaskModule, 'run').mockResolvedValue({
      success: true,
      message: '📋 [닥플 플러스 일일 자동화 리포트]',
    });
    ...
    // 인터엠디 오류 알림 + 키메디 알림 + HMP 알림 + 메디게이트 알림 + 닥플 알림 발송 검증
    expect(telegramSpy).toHaveBeenCalledTimes(5);
  ...
  it('닥플 태스크에서 예외가 발생하더라도 결과에 실패로 기록되어야 한다', async () => {
    ...
    vi.spyOn(docpleDailyTaskModule, 'run').mockRejectedValue(new Error('닥플 인증 실패'));
    ...
    expect(result.success).toBe(false);
    expect(result.message).toContain('기타 일일 퀘스트 중 일부 작업이 실패했습니다.');
    expect(result.message).toContain('닥플: ❌ 실패');

    expect(telegramSpy).toHaveBeenCalledWith(
      expect.stringContaining('❌ [기타일일퀘스트 - 닥플 일일 자동화 오류]\n사유: 닥플 인증 실패'),
    );
  });

  it('인터엠디, 메디게이트, 닥플이 silent: true일 경우 해당 텔레그램 발송을 생략해야 한다', async () => {
    vi.spyOn(intermdQuizTaskModule, 'run').mockResolvedValue({
      success: true,
    ...
    vi.spyOn(docpleDailyTaskModule, 'run').mockResolvedValue({
      success: true,
      message: '📋 [닥플]',
      silent: true,
    });
    ...
    expect(result.success).toBe(true);
    // 키메디, HMP만 발송되고 인터엠디, 메디게이트, 닥플은 silent로 인해 생략됨
  ...
  it('각 태스크의 포인트/캡슐/캐시 정보가 정상 집계되어 최종 요약 메시지에 포함되어야 한다', async () => {
    vi.spyOn(intermdQuizTaskModule, 'run').mockResolvedValue({
      success: true,
      message: '📋 [인터엠디 퀴즈]',
      options: { memberPoint: 14370 },
    });
    ...
    vi.spyOn(docpleDailyTaskModule, 'run').mockResolvedValue({
      success: true,
      message: '📋 [닥플]',
      options: { endCash: 12500, cashDiff: 100 },
    });
    ...
    expect(result.success).toBe(true);
    expect(result.message).toContain('인터엠디: ✅ 성공 (14,370P)');
    ...
    expect(opts.results.docple.cash).toBe(12500);
    expect(opts.results.docple.cashDiff).toBe(100);
  });
});
