  ...
  const mockPage: unknown = {
    ...
    locator: (selector: string) => {
      return {
        first: () => ({
          isVisible: async () => {
            if (selector.includes('회원정보수정') && currentHtml.includes('회원정보수정')) {
              return true;
            }
            if (selector.includes('input[name="') || selector.includes('#userId')) {
              return currentHtml.includes('input');
            }
            return false;
          },
        }),
...
describe('Login Check Logic Tests', () => {
  it('로그인 상태 판정 및 캐시/세션 분리 검증', async () => {
    ...
    // Case 8 — BrowserContext cache and deduplication test
    {
      ...
      const httpGetSpy = vi.spyOn(httpClientModule, 'httpGet').mockResolvedValue({
        status: 200,
        statusText: '200',
        headers: {},
