...
function parseFixtureHtml(html: string): MockNode[] {
  ...
  while ((match = listContRegex.exec(html)) !== null) {
    ...
    while ((linkMatch = linkRegex.exec(block)) !== null) {
      ...
      // 상태 확인
      const isCompletion = content.includes('ico_completion') || content.includes('신청완료');
      const isApply = content.includes('ico_apply') || content.includes('신청하기');
...
/**
 * 3. 포인트 미지급 취소선(<s>...</s>) 및 브로드캐스트 포맷팅 검증
 */
...
- 18:00~19:30. [대한혈관학회] 혈관검사 시리즈 - 2. 하지 혈관초음파의 기본과 임상에서의 활용 https://m.doctorville.co.kr/cme/seminar/5555`,
  ...
  console.log('  ✓ 당일 세미나 및 신규 세미나 취소선(<s>...</s>) 정상 포맷팅 검증 완료\n');
}
...
function testApplySeminarParsing(nodes: MockNode[]) {
  ...
  console.log('  ✓ 전체 27개 세미나 (신청완료 27/27), 정원, 시간, seminarId 파싱 검증 완료\n');
}
