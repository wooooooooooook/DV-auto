...
async function saveCookies(context) {
  try {
    ...
    console.warn('쿠키 저장 실패:', _e && _e.message ? _e.message : _e);
  }
}
...
async function saveLocalStorage(page) {
  try {
    ...
    if (fs.existsSync(LOCALSTORAGE_FILE)) {
      try {
        all = JSON.parse(fs.readFileSync(LOCALSTORAGE_FILE));
      } catch (_e) {
        all = {};
      }
    }
    ...
    console.warn('localStorage 저장 실패:', _e && _e.message ? _e.message : _e);
  }
}
...
async function loadCookies(context) {
  try {
    ...
    console.warn('쿠키 로드 실패:', _e && _e.message ? _e.message : _e);
  }
  return false;
}
...
async function loadLocalStorage(page, targetUrl) {
  try {
    ...
    console.warn('localStorage 로드 실패:', _e && _e.message ? _e.message : _e);
  }
  return false;
}
...
async function safeGoto(page, url, options = {}, retries = 2) {
  ...
  function isAbsolute(u) {
    return (
      typeof u === 'string' &&
      (/^[a-zA-Z][a-zA-Z\d+\-.]*:\/\//.test(u) || u.startsWith('about:') || u.startsWith('data:'))
    );
  }
  ...
  try {
    ...
    console.error('safeGoto: URL resolution error for', url, _e && _e.stack ? _e.stack : _e);
  }
  ...
  while (true) {
    ...
    try {
      ...
      try {
        await sendTelegram(
          `❗ safeGoto failed (${resolvedUrl}) attempt ${attempt}: ${err && err.name ? err.name : String(err)} ${err && err.code ? '(' + err.code + ')' : ''}`,
        );
      } catch (_e) {
        console.error('notify failed', _e && _e.stack ? _e.stack : _e);
      }
      if (attempt > retries)
        throw new Error(
          `safeGoto failed after ${attempt} attempts for ${resolvedUrl}: ${err && err.message ? err.message : String(err)}`,
        );
...
async function ensureLoggedIn({ page, context }) {
  ...
  try {
    if (LOGIN_URL) await loadLocalStorage(page, LOGIN_URL).catch(() => {});
  } catch (_e) {
    /* ignore */
  }
