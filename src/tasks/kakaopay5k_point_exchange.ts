...
const TARGET_URL = 'https://mcircle.bizmarketb2b.com/Goods/Content.aspx?guid=14627539&catecode=14592';
const ENTERTAINMENT_URL = 'https://www.doctorville.co.kr/entertainment/main';
const SUCCESS_TEXT = '주문이 완료되었습니다.';
const DEFAULT_POINT = '5000';
...
async function run({ page, context, maxIterations }: PlaywrightRunArgs): Promise<TaskResult> {
  ...
  const finalMaxIterations =
    maxIterations !== undefined
      ? maxIterations
      : process.env.KAKAOPAY5K_MAX_ITERATIONS !== undefined
        ? Number(process.env.KAKAOPAY5K_MAX_ITERATIONS)
        : 1;
  const refreshEvery = Number(process.env.KAKAOPAY5K_REFRESH_EVERY || process.env.NAVERPAY_REFRESH_EVERY || '3'); // 새 페이지로 리프레시할 주기
  const iterationDelayMs = Number(
    process.env.KAKAOPAY5K_ITERATION_DELAY_MS || process.env.NAVERPAY_ITERATION_DELAY_MS || '500',
  ); // 반복 간 대기 시간
  ...
  if (!name || !phone1 || !phone2 || !phone3) {
    const missing = [
      ...
      .filter(Boolean)
      .join(', ');
    const message = `카카오페이5k포인트교환 실패: 환경변수(${missing})를 확인해주세요.`;
  ...
  if (!context) {
    const message = '카카오페이5k포인트교환 실패: 로그인 확인을 위해 context가 필요합니다.';
    await sendTelegram(`❗ ${message}`).catch(() => {});
    return { success: false };
  }
  ...
  console.log(`[kakaopay5k_point_exchange] 시작 전 남은 포인트: ${startPoint}`);
  await sendTelegram(`💳 카카오페이5k포인트교환 시작 전 남은 포인트: ${startPoint}`).catch(() => {});
  ...
  try {
    ...
    while (finalMaxIterations === 0 || iteration < finalMaxIterations) {
      ...
      if (productPrice !== DEFAULT_POINT) {
        console.log(`[kakaopay5k_point_exchange] 상품금액 추출 성공: ${productPrice}원`);
      }
      ...
      if (orderCompleted) {
        successCount += 1;
        await sendTelegram(`✅ 카카오페이5k포인트교환 성공 (${successCount}회 누적, 시도 ${iteration}회)`).catch(
          () => {},
        );
      ...
      const failureShot = path.join(process.cwd(), 'screenshot', 'kakaopay5k_point_exchange_failure.png');
      await workPage.screenshot({ path: failureShot, fullPage: true }).catch(() => {});
      const endPoint = await getPoint(context);
      const message = `카카오페이5k포인트교환 실패 (시도 ${iteration}회, 성공 ${successCount}회). '${SUCCESS_TEXT}' 문구를 찾지 못했습니다.\n종료 후 남은 포인트: ${endPoint}`;
    ...
    const message =
      finalMaxIterations > 0
        ? `카카오페이5k포인트교환 완료: 설정된 ${finalMaxIterations}회 반복 종료 (성공 ${successCount}회).\n종료 후 남은 포인트: ${endPoint}`
        : `카카오페이5k포인트교환 종료: 성공 ${successCount}회 후 반복이 중단되었습니다.\n종료 후 남은 포인트: ${endPoint}`;
    await sendTelegram(`✅ ${message}`).catch(() => {});
    return { success: true };
  } catch (error) {
    const errorShot = path.join(process.cwd(), 'screenshot', 'kakaopay5k_point_exchange_error.png');
    await workPage.screenshot({ path: errorShot, fullPage: true }).catch(() => {});
    const endPoint = await getPoint(context);
    const message = `카카오페이5k포인트교환 오류 발생 (성공 ${successCount}회): ${
