...
function appendNewSeminarsToHistory(items: Array<SeminarListItem & { seminarId: string | null }>, detectedDate: string): void {
  ...
  storage.set(NEW_SEMINAR_HISTORY_KEY, history.filter((entry) => {
...
async function refreshAdvancedPointStatus(context: PlaywrightRunArgs['context']): Promise<Map<string, SeminarPointStatus>> {
  ...
  for (const seminar of targets) {
    ...
    statuses.set(seminar.url, {
      pointPaid: result.type === '적립', point: result.point, pointText: result.pointText,
      pointDate: result.date, pointContent: result.content, pointCheckedAt: checkedAt,
    });
  }
  return statuses;
}
...
async function run({ page, context }: PlaywrightRunArgs, options: ApplySeminarOptions = {}): Promise<TaskResult> {
  ...
  try {
    ...
    if (storedSeminars.length > 0) {
      ...
      if (newlyAdded.length > 0) {
        for (const item of newlyAdded) {
          ...
          if (!isPointExcluded) { await page.waitForTimeout(800); isPointExcluded = await isSurveyPointExcludedSeminar(page.context(), link); }
        ...
        const merged = [...baseSeminars]; const existingUrls = new Set(baseSeminars.map((item) => item.url));
        for (const item of newlyAddedWithFlags) if (!existingUrls.has(item.url)) { merged.push(item); existingUrls.add(item.url); }
    ...
    const finalSeminarsToStore: SeminarListItem[] = normalizedCurrentSeminars.map((item) => {
      ...
      return { ...item, isPointExcluded: stored?.isPointExcluded ?? newlyAdded?.isPointExcluded, isAdvancedSurvey: item.isAdvancedSurvey,
        pointPaid: stored?.pointPaid, point: stored?.point, pointText: stored?.pointText, pointDate: stored?.pointDate, pointContent: stored?.pointContent, pointCheckedAt: stored?.pointCheckedAt };
    });
    ...
    if (!screenshotPath) { const baseScreenshotDir = path.join(process.cwd(), 'screenshot'); await fs.mkdir(baseScreenshotDir, { recursive: true }); screenshotPath = path.join(baseScreenshotDir, 'apply_seminar_error.png'); await page.screenshot({ path: screenshotPath, fullPage: false }).catch(() => {}); }
