...
const pointExchange: Task = {
  ...
  run: async (ctx) => {
    ...
    try {
      await utils.ensureLoggedIn({ page, context });
      return await pointExchangeTask.run({ page, context, maxIterations: ctx.maxIterations, args: ctx.args });
    } finally {
      await browser.close();
    }
  },
};
taskRegistry.registerTask(pointExchange);
